...
{
    public class NewPr : IObjectSetProvider
    {
        public ObjectSet GetSet()
        {
            var features = Enumerable.Range(0, 80).Select(s => new Feature() { IsContinuous = false, Name = $"Ft {s:000000}" });

            var objects = new List<ObjectInfo>();
            int ind = 0;
            // using (var writer = new StreamWriter(Path.Combine("Data", "Dorothea", "thrombin_unique_set_True.data.new")))
            using (var reader = new StreamReader(Path.Combine("Data", "Train", "newPr.csv")))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    {
                        ClassValue = int.Parse(line[80]),
                        Data = Array.ConvertAll(line.Take(80).ToArray(), x => decimal.Parse(x)),
                        Index = ind++
                    };
